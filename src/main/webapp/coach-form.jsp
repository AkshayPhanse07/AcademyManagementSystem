...
<title>${coach != null ? 'Edit Cricket Coach' : 'Add New Cricket Coach'}</title>
<!-- Bootstrap CSS -->
<link
...
<body>
	...
	<div class="container ">
		<h1 class="mb-4 text-center">${coach != null ? 'Edit Cricket Coach' : 'Add New Cricket Coach'}</h1>

		<!-- Dynamic form action: Insert or Update -->
		<form action="coach?action=${coach != null ? 'update' : 'insert'}"
			method="post">
			<div class="form-group">
				<label for="coachid">Coach ID:</label> <input type="text"
					id="coachid" name="coachid" class="form-control"
					value="${coach.coachid}" ${coach != null ? 'readonly' : ''}
					required>
			</div>
			...
			<div class="form-group">
				<label for="coachType">Coach Type:</label> <select id="coachType"
					...
					<option value="Batting"
						${coach.coachType == 'Batting' ? 'selected' : ''}>Batting
						Coach</option>
					<option value="Bowling"
						${coach.coachType == 'Bowling' ? 'selected' : ''}>Bowling
						Coach</option>
					<option value="Fielding"
						${coach.coachType == 'Fielding' ? 'selected' : ''}>Fielding
						Coach</option>
					<option value="All-Rounder"
						${coach.coachType == 'All-Rounder' ? 'selected' : ''}>All-Rounder
						Coach</option>
				</select>
			</div>
			...
			<button type="submit" class="btn btn-primary">${coach != null ? 'Update Coach' : 'Insert Coach'}
			</button>
